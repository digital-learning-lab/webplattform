...
def send_mail(
    ...
    logger.debug(
        "Email sending initiated - attempt nr. {}".format(self.request.retries)
    )
    ...
    try:
        ...
        if email:  # send directly to provided email addresses
            ...
            recipients = list(
                DllUser.objects.filter(pk__in=recipient_ids).values_list(
                    "email", flat=True
                )
            )
        ...
        logger.warning(
            "Sending email failed. Event type code: {}".format(event_type_code)
        )
        logger.exception(e)
