...
class BaseTestCase(TestCase):
    def setUp(self):
        ...
        ContentLink.objects.create(
            url="https://www.foo.org", name="FooLink", type="href", content=self.content
        )
        ...
        ContentFile.objects.create(
            file=filer_file, title="BarFile", content=self.content
        )
...
class ContentEditTests(BaseTestCase):
    ...
    def test_user_can_edit_own_content(self):
        self.assertTrue(
            self.author.has_perm("content.change_teachingmodule", self.content)
        )

    def test_other_user_cannot_edit_foreign_content(self):
        self.assertFalse(
            self.other_author.has_perm("content.change_teachingmodule", self.content)
        )

    def test_coauthor_has_edit_permission(self):
        self.assertTrue(
            self.co_author.has_perm("content.change_teachingmodule", self.content)
        )
        self.assertFalse(
            self.other_author.has_perm("content.change_teachingmodule", self.content)
        )
...
class ReviewSubmissionTests(BaseTestCase):
    ...
    def test_reviewer_can_edit_review(self):
        self.assertTrue(
            self.bsb_reviewer.has_perm("content.change_review", self.content.review)
        )
...
class ReviewAcceptTests(BaseTestCase):
    ...
    def test_content_relations_have_different_pks(self):
        ...
        self.assertFalse(
            set(self.draft.contentlink_set.all())
            == set(self.public.contentlink_set.all())
        )
        self.assertFalse(
            set(self.draft.contentfile_set.all())
            == set(self.public.contentfile_set.all())
        )

    def test_public_and_draft_have_same_authors(self):
        self.assertEqual(self.draft.author, self.public.author)
        self.assertEqual(
            set(self.draft.co_authors.all()), set(self.public.co_authors.all())
        )
    ...
    def test_public_and_draft_have_same_related_content(self):
        ...
        self.assertEqual(
            set(published_related_content), set(self.public.related_content.all())
        )
