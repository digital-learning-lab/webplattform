<template>
  <app-content-submission-form :errors="errors" :mode="mode" :loading="loading" :saved="saved" @update="updateContent" @create="createContent" @preview="goToPreview" @delete-warning="showDeleteWarning" :data="data" @delete="deleteContent" @submit="submitContent" @update-review="updateReview" @approve-review="approveContent" @decline-review="declineContent">
    ...
    <div v-if="mode === 'edit' || mode === 'review'">
      <app-file-input id="image" :readonly="readonly" :review="review" :review-value.sync="reviewValue.image" :error="errorFields.includes('image')" label="Anzeigebild" file-label="Bild wählen" :value.sync="previewImage" :image="data.image" :help-text="getHelpText('image')" :hintText="imageHintText"></app-file-input>
      ...
      <app-dropdown id="tools" :readonly="readonly" :review="review" :review-value.sync="reviewValue.tools" :error="errorFields.includes('tools')" label="Verwendete Tools" :value.sync="data.tools" fetch-url="/api/tools" :multiple="true" :help-text="getHelpText('tools')"></app-dropdown>
      <app-dropdown id="trends" :readonly="readonly" :review="review" :review-value.sync="reviewValue.trends" :error="errorFields.includes('trends')" label="Passende Trends" :value.sync="data.trends" fetch-url="/api/trends" :multiple="true" :help-text="getHelpText('trends')"></app-dropdown>
      ...
      <app-list-input id="target-group" :readonly="readonly" :review="review" :review-value.sync="reviewValue.target_group" :error="errorFields.includes('target_group')" label="Zielgruppe" :list.sync="data.target_group" :initial="data.target_group" :help-text="getHelpText('target_group')"></app-list-input>
      ...
      <app-list-input id="publisher" :readonly="readonly" :review="review" :review-value.sync="reviewValue.publisher" :error="errorFields.includes('publisher')" label="Herausgeber_in" :list.sync="data.publisher" :initial="data.publisher" :help-text="getHelpText('publisher')"></app-list-input>
      <app-list-input id="goals" :readonly="readonly" :review="review" :review-value.sync="reviewValue.learning_goals" :error="errorFields.includes('learning_goals')" label="Zielsetzung" :list.sync="data.learning_goals" :initial="data.learning_goals" :help-text="getHelpText('learning_goals')"></app-list-input>
