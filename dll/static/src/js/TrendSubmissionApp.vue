<template>
  <app-content-submission-form :errors="errors" :mode="mode" :loading="loading" :saved="saved" @update="updateContent" @create="createContent">
      ...
      <app-text-input id="title" label="Titel des Trends" :value.sync="data.name" :required="true" :character-counter="true" :maximal-chars="140"></app-text-input>
    <div v-if="mode === 'edit'">
      ...
      <app-select id="category" label="Kategorie" :options="categoryOptions" :value.sync="data.category" :default-val="data.category"></app-select>
      <app-list-input id="target-group" label="Zielgruppe" :list.sync="data.target_group" :initial="data.target_group"></app-list-input>
      <app-select id="language" label="Sprache" :options="languageOptions" :value.sync="data.language" :default-val="data.language"></app-select>
      <app-list-input id="publisher" label="Herausgeber_in" :list.sync="data.publisher" :initial="data.publisher"></app-list-input>
      <app-list-input id="goals" label="Zielsetzung" :list.sync="data.learning_goals" :initial="data.learning_goals"></app-list-input>
      <app-text-area id="central-contents" label="Zentrale Inhalte" :value.sync="data.central_contents" :character-counter="true" :maximal-chars="1300"></app-text-area>
      <app-text-area id="additional-info" label="Hintergrundinformationen" :required="true" :value.sync="data.additional_info" :character-counter="true" :maximal-chars="1500" :rows="10"></app-text-area>
      <app-select id="license" label="Lizenz" :options="licenseOptions" :default-val="data.licence" :value.sync="data.licence"></app-select>
      <app-text-area id="citation-info" label="Zitierhinweis" :value.sync="data.citation_info" :character-counter="true" :maximal-chars="200"></app-text-area>

      <app-links-input id="websites" :links.sync="data.mediaLinks" label="Website" type="href"></app-links-input>
      <app-links-input id="additionalLinks" :links.sync="data.literatureLinks" label="Weitere Links" type="href"></app-links-input>
