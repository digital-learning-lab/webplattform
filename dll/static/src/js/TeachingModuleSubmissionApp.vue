<template>
  <app-content-submission-form :errors="errors" :mode="mode" :loading="loading" :saved="saved" @update="updateContent" @create="createContent" @preview="goToPreview" @delete-warning="showDeleteWarning" :data="data" @delete="deleteContent" @submit="submitContent" @update-review="updateReview" @approve-review="approveContent" @decline-review="declineContent">
    ...
    <div v-if="mode === 'edit' || mode === 'review'">
      ...
      <app-text-area id="teaser" :readonly="readonly" :review="review" label="Teaser" :required="true" :value.sync="data.teaser" :review-value.sync="reviewValue.teaser" :rows="3" :help-text="getHelpText('teaser')" :character-counter="true" :maximal-chars="140"></app-text-area>
      ...
      <app-dropdown id="subCompetences" :readonly="readonly" :review="review" label="Detaillierte Kompetenzbeschreibungen" :value.sync="data.sub_competences" :review-value.sync="reviewValue.sub_competences" :disabled="!data.competences.length" fetch-url="/api/sub-competences" :prefetch="true" :params="{competences: data.competences}" :multiple="true" :help-text="getHelpText('sub_competences')"></app-dropdown>
      <app-list-input id="estimatedTime" :readonly="readonly" :review="review" label="Zeitumfang der Durchführung" :list.sync="data.estimated_time" :review-value.sync="reviewValue.estimated_time" :initial="data.estimated_time" :help-text="getHelpText('estimated_time')" :character-counter="true" :maximal-chars="250"></app-list-input>
      <app-list-input id="goals" :readonly="readonly" :review="review" label="Ziele" :list.sync="data.learning_goals" :review-value.sync="reviewValue.learning_goals" :initial="data.learning_goals" :help-text="getHelpText('learning_goals')"></app-list-input>
      <app-range-input id="classes" :readonly="readonly" :review="review" label="Jahrgangsstufe" label-from="Von" label-to="Bis" type="number" :range.sync="data.school_class" :review-value.sync="reviewValue.school_class" :min="1" :max="13" :help-text="getHelpText('school_class')"></app-range-input>
      <app-list-input id="subject-of-tuition" :readonly="readonly" :review="review" label="Informationen zum Unterrichtsgegenstand" :list.sync="data.subject_of_tuition" :review-value.sync="reviewValue.subject_of_tuition" :initial="data.subject_of_tuition" :help-text="getHelpText('subject_of_tuition')"></app-list-input>
      <app-list-input id="expertise" :readonly="readonly" :review="review" label="Fachkompetenzen" :list.sync="data.expertise" :review-value.sync="reviewValue.expertise" :initial="data.expertise" :help-text="getHelpText('expertise')"></app-list-input>
      <app-list-input id="equipment" :readonly="readonly" :review="review" label="Medienausstattung" :list.sync="data.equipment" :review-value.sync="reviewValue.equipment" :initial="data.equipment" :help-text="getHelpText('equipment')"></app-list-input>
      ...
      <app-links-input id="mediaLinks" :readonly="readonly" :review="review" :links.sync="data.mediaLinks" :review-value.sync="reviewValue.mediaLinks" label="Links zu Audio- und Videomedien" :type="'video'" :help-text="getHelpText('contentlink')" :types="true"></app-links-input>
      <app-links-input id="literatureLinks" :readonly="readonly" :review="review" :links.sync="data.literatureLinks" :review-value.sync="reviewValue.literatureLinks" label="Weiterführende Literatur und Links" :help-text="getHelpText('contentlink')" :types="true"></app-links-input>
