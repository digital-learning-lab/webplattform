<template>
  <app-content-submission-form :errors="errors" :mode="mode" :loading="loading" :saved="saved" @update="updateContent" @create="createContent" @preview="goToPreview" @delete-warning="showDeleteWarning" :data="data" @delete="deleteContent" @submit="submitContent" @update-review="updateReview" @approve-review="approveContent" @decline-review="declineContent">
    ...
    <div v-if="mode === 'edit' || mode === 'review'">
      ...
      <app-dropdown id="teaching-modules" :readonly="readonly" :review=review label="Passende Unterrichtsbausteine" :value.sync="data.teaching_modules" :review-value.sync="reviewValue.teaching_modules" :error="errorFields.includes('teaching_modules')" fetch-url="/api/unterrichtsbausteine" :multiple="true" :help-text="getHelpText('teaching_modules')"></app-dropdown>
      <app-dropdown id="tools" :readonly="readonly" :review="review" label="Verwendete Tools" :value.sync="data.tools" :review-value.sync="reviewValue.tools" :error="errorFields.includes('tools')" fetch-url="/api/tools" :multiple="true" :help-text="getHelpText('tools')"></app-dropdown>
      <app-dropdown id="trends" :readonly="readonly" :review="review" label="Passende Trends" :value.sync="data.trends" :review-value.sync="reviewValue.trends" :error="errorFields.includes('trends')" fetch-url="/api/trends" :multiple="true" :help-text="getHelpText('trends')"></app-dropdown>
      ...
      <app-links-input id="websites" :readonly="readonly" :review="review" :links.sync="data.websites" :review-value.sync="reviewValue.websites" :error="errorFields.includes('websites')" label="Website" :type="'null'" :help-text="getHelpText('websites')"></app-links-input>
      <app-list-input id="pro" :readonly="readonly" :review="review" label="Vorteile" :list.sync="data.pro" :review-value.sync="reviewValue.pro" :error="errorFields.includes('pro')" :initial="data.pro" :help-text="getHelpText('pro')"></app-list-input>
      <app-list-input id="contra" :readonly="readonly" :review="review" label="Nachteile" :list.sync="data.contra" :review-value.sync="reviewValue.contra" :error="errorFields.includes('contra')" :initial="data.contra" :help-text="getHelpText('contra')"></app-list-input>
      ...
      <app-links-input id="mediaLinks" :readonly="readonly" :review="review" :links.sync="data.mediaLinks" :review-value.sync="reviewValue.mediaLinks" :error="errorFields.includes('mediaLinks')" label="Links zu Audio- und Videomedien" :type="'video'" :help-text="getHelpText('contentlink')"></app-links-input>
      <app-links-input id="literatureLinks" :readonly="readonly" :review="review" :links.sync="data.literatureLinks" :review-value.sync="reviewValue.literatureLinks" :error="errorFields.includes('literatureLinks')" label="Text-Anleitung" :help-text="getHelpText('contentlink')"></app-links-input>
